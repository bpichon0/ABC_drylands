...
p=ggplot(read.table("../Data/Data_Biocom/biocom_data.csv",sep=";"))+
  geom_histogram(aes(x=Nbpixels),fill=alpha("blue",.5))+
  the_theme+
  labs(x='# of pixels',y="Count")
ggsave("../Figures/Empirical_data/Comparizon_models_data/Understanding_differences/Spatial_resolution_data.pdf",p,width=6,height = 3)
  ...
  for (i in 1:3){
    assign(paste0("p",i),
           d_tot%>%
             ...
             geom_hline(yintercept = 0, lty = 2) +
             geom_vline(xintercept = 0, lty = 2) +
             geom_point(aes(x = PC1, y = PC2, color = NBpixels,fill=NBpixels,size=Type),alpha=.5)+
...
for (k in list_folders){
  ...
  d=add_column(d,Pooling=rep(c(2,1,.5),nrow(d)/3))%>%
    add_column(., Model=tail(strsplit(k,"/")[[1]],1))%>%
    filter(., rho_p != 0)
...
d_sim_decrease_res=mutate(d_simu,
             Spectral_ratio=log(Spectral_ratio),clustering=log(clustering),
...
#  sample_n(n_keep)%>%
  ...
  summarise(., .groups = "keep",mean_value=mean(value,na.rm = T))%>%
  ggplot(.)+
  geom_line(aes(x=Pooling,y=mean_value,group=interaction(Model),color=Model),lwd=1,alpha=1)+
  geom_point(aes(x=Pooling,y=mean_value,color=Model),fill="white",shape=21,size=2.5)+
...
d_sim_data=rbind(rbind(d_sim,d_sim_decrease_res%>%add_column(., p=NA,q=NA)%>%relocate(p,.after=PL_expo)%>%
                         relocate(q,.after=p)%>%dplyr::select(., -ID)),
                 d_biocom[which(d_biocom$Nbpixels<80000),17:ncol(d_biocom)]%>% #keeping only the data with relatively low quality
                   add_column(., p=NA,q=NA, Pooling=NA,Model=NA)%>%
                   mutate(., Spectral_ratio=log(Spectral_ratio),clustering=log(clustering)))
...
for (i in 1:3){
  assign(paste0("p",i),
         d_sim_data%>%
           mutate(., Pooling=recode_factor(Pooling,"1/4"='x4',"1/3"="x3","1/2"="x2","1"="No change","0.5"="/2","2"="x2"))%>%
           add_column(., PC1=res.pca$ind$coord[,axes_for_plot$x[i]],PC2=res.pca$ind$coord[,axes_for_plot$y[i]])%>%
           ggplot(.) +
           geom_hline(yintercept = 0, lty = 2) +
           geom_vline(xintercept = 0, lty = 2) +
           geom_point(aes(x = PC1, y = PC2, color = Pooling,fill=Pooling),alpha=.5)+
           scale_color_manual(values=viridis_pal()(6),na.value="black")+
           scale_fill_manual(values=viridis_pal()(6),na.value="black")+
           labs(x=paste0("PC ",axes_for_plot$x[i]," (",round(res.pca$eig[axes_for_plot$x[i],2], 1)," %)"),
                y=paste0("PC ",axes_for_plot$y[i]," (",round(res.pca$eig[axes_for_plot$y[i],2], 1)," %)"),color="Change in resolution",fill="")+
           ggtitle("")+guides(shape="none")+
           theme_classic()+theme(legend.position = "bottom")+
...
p_1=ggarrange(ggarrange(p1+theme(legend.position = "none"),
                        ...
                        ncol=3,align = "hv"),ggarrange(ggplot()+theme_void(),get_legend(p2),ggplot()+theme_void(),ncol=3,widths = c(.3,1,.3)),
              nrow=2,heights = c(1,.1))
ggsave(paste0("../Figures/Spatial_resolution/Model/PCA_with_decreasing_resolution.pdf"),
       p_1,width = 11,height = 5)




## >> 4) Which are the outsiders in the PCA ? (low cover sites) ----
...
p=ggplot(d_biocom%>%mutate(., Spectral_ratio=log(Spectral_ratio),clustering=log(clustering))%>%
         melt(., measure.vars=colnames(d_biocom)[17:25]))+
  geom_density(aes(x=value,fill=Far),alpha=.4)+
  facet_wrap(.~variable,scales = "free")+
  labs(x="Value",y="Density",fill="outsider site ?")+
  theme_classic()+
  theme(legend.position = "bottom")

ggsave(paste0("../Figures/Spatial_resolution/Model/Outsiders_PCA_model_data.pdf"),
       p,width = 7,height = 6)
...
id=85
plot_distr(spatialwarnings::patchdistr_sews(Get_empirical_site(id)==1,xmin = 1),best_only=F)
...
plot_distr(spatialwarnings::patchdistr_sews(pooling(Get_empirical_site(id),1)==1,xmin = 9))

id=87
plot_distr(spatialwarnings::patchdistr_sews(Get_empirical_site(id)==1,xmin = 1))

tpl_fit(patchdistr_sews(Get_empirical_site(id)==1)$psd_obs,xmin = 1)
exp_fit(Get_empirical_site(id)==1,xmin = 1)

as.data.frame(spatialwarnings::patchdistr_sews(Get_empirical_site(id)==1,xmin = 1))


sd(patchdistr_sews(inverse_pooling(Get_empirical_site(id),2)==1)$psd_obs)/mean(patchdistr_sews(inverse_pooling(Get_empirical_site(id),2)==1)$psd_obs)
...
for (sample_id in list_sample){
  ...
  for (scale_obs in c(4,3,2)){ #we include the scale of observation as a parameter of the model
    ...
    if (any(is.na(target))){
      ...
      which_na=which(is.na(target))
      abc_sim=abc(target=target[-which_na,4:12],
                  param = all_sim[,1:3],sumstat = all_sim[,c(4:11)[-which_na]],
...
d_RMSE_param=read.table("../Data/Step10_ABC_scale/Retrieving_parameters_different_resolution_RMSE_param.csv",sep=";")
colnames(d_RMSE_param)[1:3]=c("Scale obs","p","q")
p1=ggplot(d_RMSE_param%>% #we remove the scale of observation
  ...
  facet_wrap(.~variable,labeller = label_bquote(cols = .(as.character(variable))))+
  labs(x="Method ABC (post-processing)",y="NRMSE",color="")+
  the_theme+
  guides(Method="none")+
...
#write.table(x_y_param,"../Data/Step10_ABC_scale/Retrieving_parameters_different_resolution_x_y.csv",sep=";")
x_y_param=read.table("../Data/Step10_ABC_scale/Retrieving_parameters_different_resolution_x_y.csv",sep=";")
p2=ggplot(x_y_param%>% #we remove the scale of observation
            filter(., Method=="NeuralNet")%>%
           melt(., id.vars=c("Site_ID","Method","Pooling","Pooling.1","Type"))%>%
         filter(., Type=="Sim"))+
...
d_sim=list.files("../Data/Step12_Prediction/Space_param",".csv")
d=tibble()
for (i in d_sim){
  d=rbind(d,read.table(paste0("../Data/Step12_Prediction/Space_param/",i),sep=","))
}
colnames(d)=c("p","q","Cover","Branch")
d_param_infer_rej=readRDS("../Data/Step11_Inferrence/Inferred_param_sites_Eby_feedbackrej.rds")
...
for (site_id in 1:ncol(d_param_infer_rej)){
  ...
  close_p=unique(d$p[which(abs(d$p - p_site) == min(abs(d$p - p_site)))])
  close_q=unique(d$q[which(abs(d$q - q_site) == min(abs(d$q - q_site)))])

  dist_tipping=abs(diff(range(d$p[which(d$q==close_q & d$Cover!=0 & d$p<= close_p)])))
...
ggplot(d_tipping)+
  geom_point(aes(x=MF,Tipping))+
  geom_smooth(aes(x=MF,Tipping))
