...
method_abc="rejection"
  `%!in%` = Negate(`%in%`)  
  data_guichard=read.table("./Stats_guichard.csv",sep=",")
  colnames(data_guichard)=c("d","a0","a2","rho_p","nb_neigh","clustering","skewness","variance","moran_I","Spectral_ratio","PLR","PL_expo","cv_psd","fmax_psd")
  print(colnames(data_guichard))
  print(dim(data_guichard)) 
  ...
  d_param_infer_NN=array(0,c(n_sim_kept,nrow(data_guichard),3))
  d_param_infer_rej=array(0,c(n_sim_kept,nrow(data_guichard),3))
  ...
  for (empirical_id in 1:n_tot){
    print(empirical_id)
    target=data_guichard[empirical_id,4:14]
