  ...
  for (size_step1 in c(1000,3000)){
    ...
    for (method_abc in c("loclinear","neuralnet")){
      ...
      for (preprocessing in c("PLS_BoxCox","BoxCox","None")){
          ...
          if (preprocessing %in% c("BoxCox", "PLS_BoxCox")){ #Applying the two step procedure used in Siren MEE paper : Don't know whether it make sense in our case. TO discuss Monday
            ...
            if (any(is.na(mat_sumstat_pls[n_cross,]))){
              ...
              cross_valid1=abc(target = mat_sumstat_pls[n_cross,-which_na],
                              ...
                              tol = size_step1/nrow(matrix_param),method = "rejection") #we keep the 1000 closest simulations for the first step
        ...
        write.table(d_NRMSE_param,paste0("../Data_new/RMSE_param_",preprocessing,
        ...
        write.table(d_NRMSE_sumstat,paste0("../Data_new/RMSE_sumstat_",preprocessing,
                                           "_",method_abc,"_optim_lambda_",ifelse(optim_lambda,"yes","no"),"_N1_",size_step1,".csv"),sep=";")
...
for (method_pre in c("PLS","NoPLS")){
  ...
  for (size_hidden in seq(5,25,by=5)){
    ...
    for (rep_network in c(10,20,30)){
      ...
      write.table(d_NRMSE_param,paste0("../Data_new/RMSE_hidden_preprocessing_",method_pre,"_",
...
## >> 3) Selecting the best summary statistics ----
...
for (which_sumstat in 1:length(all_name)){
  ...
  write.table(d_NRMSE_param,paste0("../Data_new/RMSE_selecting_sumstat_,",name_removal,".csv"),sep=";")
...
write.table(d_RMSE_param,"../Data_new/Retrieving_parameters_different_resolution_RMSE_param.csv",sep=";")
write.table(x_y_param,"../Data_new/Retrieving_parameters_different_resolution_x_y.csv",sep=";")
...
for (empirical_id in 1:345){
  ...
  target=d_biocom[empirical_id,14:24]
  
  if (any(is.na(target))){
    
    which_na=which(is.na(target))
    abc_sim=abc(target=target[-which_na],
                param = d_sim[,1:(n_param)],sumstat = d_sim[,sumstat_kept[-which_na]],
    ...
    abc_sim=abc(target=target,
                param = d_sim[,1:(n_param)],sumstat = d_sim[,sumstat_kept],
                tol = 50/nrow(d_sim),method="rejection")
...
write_rds(d_param_infer_NN,paste0("../Data_new/Inferred_param_sites_NN.rds"))
write_rds(d_param_infer_rej,paste0("../Data_new/Inferred_param_sites_Rej.rds"))
write.table(x_y_stat,paste0("../Data_new/x_y_obs_sim_stat.csv"),sep=";")
...
# ---------------------- Step XXX: Relevant spatial statistics using PCA & x-y obs-sim ----


#Simulations and empirical data
d=read.table("../Data_new/All_new_sim.csv",sep=";")
d_biocom=read.table("../Data_new/biocom_data.csv",sep=";")
d_sim_data=rbind(d[,c(3:13,14)]%>%add_column(., Model=NA),
                 d_biocom[,14:ncol(d_biocom)]%>% #keeping only the data with relatively low quality
                   add_column(., Pooling="Data",Model=NA))

name_plot=c("no_cv_fmax","no_PL_PLR","not_the_4","only_fmax","all")

for (id_plot in c(1:5)){
  
  sumstat_name=colnames(d_sim_data)[1:11]
  
  if (id_plot==1){
    sumstat_name=sumstat_name[-c(10:11)]
  }else if (id_plot==2){
    sumstat_name=sumstat_name[-c(8,9)]
  }else if (id_plot==3){
    sumstat_name=sumstat_name[-c(8:11)]
  }else if (id_plot==4){
    sumstat_name=sumstat_name[-c(8:10)]
  }
  ...
  ggsave(paste0("./PCA_sumstat/PCA_spatial_resolution_test_model_and_data_",name_plot[id_plot],".pdf"),p,width = 11,height = 5)
...
for (id_plot in c(1:6)){
  ...
  name_plot=c("no_cv_fmax","no_PL_PLR","not_the_4","only_fmax","no_PL","all")
  ...
  for (empirical_id in 1:345){
    ...
    target=d_biocom[empirical_id,colnames(d_biocom) %in% colnames(d_sim)[sumstat_kept]]
  ...
  write.table(x_y_stat,paste0("../Data_new/d/x_y_obs_sim_stat_",name_plot[id_plot],".csv"),sep=";")
  
}

pdf("../Figures/Final_figs/test.pdf")

for (k in 1:length(list.files("../Data_new/d"))){
  x_y_stat=read.table(paste0("../Data_new/d/",list.files("../Data_new/d")[k]),sep=";")
  
  if (k==1){
    save_name=colnames(x_y_stat)[1:(ncol(x_y_stat)-3)]
  }
  
  nplot=ncol(x_y_stat)-3
    
  par(mfrow=c(3,4))
  for (i in 1:12){
    d_sim=filter(x_y_stat,Type=="Sim")
    d_obs=filter(x_y_stat,Type=="Obs")
    if (save_name[i] %in% colnames(x_y_stat)){
      plot(x=d_sim[,save_name[i]],xlab="Sim",ylab="Obs",main=save_name[i],
           y=d_obs[,save_name[i]],col="gray")
      abline(a=0,b=1)
      
    }else {
      image(matrix(1,1,1),col="white")
    }
