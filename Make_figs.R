...
size_tip=pred%>%group_by(., ID_sim)%>%
  filter(., cover>0)%>%
  dplyr::summarise(.,size_tipping=min(cover),p=p[which(cover==min(cover))],.groups = "keep")
...
## >> Fig. 2 Predictions and predictors cover vs sp. struct ----
...
pred=tibble()
for (k in site){
  d=read.table(paste0("../Data_new/Prediction/Dist_tipping_",k,".csv"),sep=",")%>%
...
for (k in unique(pred$Site)){
  if (length(unique(pred$ID_sim[which(pred$Site==k)]))==2){
    pred=pred[-which(pred$ID_sim != unique(pred$ID_sim[which(pred$Site==k)])[1] & pred$Site==k),]
  }
}
p1=ggplot(NULL)+
  geom_point(data=pred,aes(p,y=cover,shape=as.factor(Site),
                           fill=as.factor(Site),
                           color=as.factor(Site)),size=2)+
  the_theme+
  scale_shape_manual(values=c(25,11,16))+
  scale_color_manual(values=c("gray60","black","orange"))+
  scale_fill_manual(values=c("gray60","white","orange"))+
  labs(y="Vegetation cover",x="Distance to desertification point (p-pcrit)",shape="")+
  guides(color = "none",size="none")+
  theme(legend.position = "none")+
  xlim(-0.02,.25)

ggsave("../Figures/Final_figs/Posterior_predictionsa.pdf",p1,width = 5,height = 3)
...
d_summarized=d%>%
 dplyr::group_by(., Site,MF,aridity,Sand)%>%
  ...
  add_column(.,ID=1:nrow(.),Cover=d_biocom$Cover[keep_sites],
             ...
             mean_p = colMeans(post_param[,keep_sites]),
             mean_q = colMeans(post_param[,keep_sites+345]),
             sd_p = apply(post_param[,keep_sites],2,sd),
             sd_q = apply(post_param[,keep_sites+345],2,sd))
...
name_AIC_plot=c("Abs dist.)","Rela dist.)","Size tipping.)")
for (k in 1:3){
  
  assign(paste0("p4_",k),
         ggplot(d_mod%>%
                  melt(., measure.vars=c("AIC"))%>%
                  dplyr::filter(., Stability==unique(.$Stability)[k]))+
           ...
           labs(x=paste0("Model AIC (",name_AIC_plot[k]),y="Count",fill="")+
...
p_tot=ggarrange(
  ...
  ggarrange(p4_2+theme(legend.position = c(.625,.9)),
            p4_3+theme(legend.position = "none")+labs(y="Count \n "),
            ncol=2,nrow=1,labels=c("b",""),hjust = c(-7,-8)),
  nrow=2,heights = c(1,.66))

ggsave("../Figures/Final_figs/Posterior_predictionsb.pdf",p_tot,width = 8,height = 6)
...
d_mod=read.table("../Data_new/Prediction/Drivers_stability_metrics.csv",sep=";")

for (k in 1:3){
  assign(paste0("p_",k),
         ggplot(d_mod%>%melt(., id.vars=c("Param"))%>%
...
d_importance=read.table("../Data_new/Prediction/Importance_drivers_stability_metrics.csv",sep=";")%>%
  filter(., Response_var %in% c("rela_dist","Size_tipping","q"))

mean_imp=d_importance%>%
  dplyr::group_by(., Response_var)%>%
  dplyr::summarise(., .groups = "keep",R2m=mean(R2m),R2c=mean(R2c),Facilitation=mean(Facilitation),
                   SR=mean(SR),Cover=mean(Cover),Aridity=mean(Aridity),Sand=mean(Sand))

sd_imp=d_importance%>%
  dplyr::group_by(., Response_var)%>%
  dplyr::summarise(., .groups = "keep",R2m=sd(R2m),R2c=sd(R2c),Facilitation=sd(Facilitation),
                   SR=sd(SR),Cover=sd(Cover),Aridity=sd(Aridity),Sand=sd(Sand))

Importance_merged=mean_imp

Importance_merged=rbind(cbind("Response_var"="Averaged across metrics",
                              "R2m"=0,"R2c"=0,as_tibble(t(colMeans(mean_imp[,4:ncol(mean_imp)])))),
                        Importance_merged)%>%
  mutate(., R2m=0)%>%
  melt(., measure.vars=colnames(.)[c(2,4:(ncol(.)))])%>%
  mutate(., variable=recode_factor(variable,"SR"="Species richness","R2m"="R2"),
         Response_var=recode_factor(Response_var,
                                    #"abs_dist"="Absolute distance",
                                    "q" ="Parameter q, (spatial structure)",
                                    "rela_dist"="Distance to desertification point",
                                    "Size_tipping" ="Size of the tipping point"))%>%
  #arranging order for predictors (xaxis)
  add_column(., Order_pred=sapply(1:nrow(.),function(x){
    if (.$variable[x]=="Cover") return(1)
    if (.$variable[x]=="Sand") return(2)
    if (.$variable[x]=="Aridity") return(3)
    if (.$variable[x]=="Species richness") return(4)
    if (.$variable[x]=="Facilitation") return(5)
    if (.$variable[x]=="R2") return(6)
    
  }))%>%
  mutate(variable = fct_reorder(variable, Order_pred))%>%
  #arranging order for response var (yaxis)
  add_column(., Order_resp=sapply(1:nrow(.),function(x){
    if (.$Response_var[x]=="Averaged across metrics") return(4)
    if (.$Response_var[x]=="Distance to desertification point") return(3)
    if (.$Response_var[x]=="Size of the tipping point") return(2)
    if (.$Response_var[x]=="Parameter q, (spatial structure)") return(1)
    
  }))%>%
  mutate(Response_var = fct_reorder(Response_var, Order_resp))

n_var=length(unique(Importance_merged$variable))
n_metric=length(unique(Importance_merged$Response_var))

p=ggplot(Importance_merged)+
  geom_tile(aes(x=variable,y=Response_var,fill=value))+
  geom_text(data=tibble(x=n_var,y=1:(n_metric-1),label=round(mean_imp$R2m,2)),#round(mean_imp$R2m[c(5,3,1,3,2)],2)),
            aes(x=x,y=y,label=paste0(label)),size=3)+
  geom_text(data=tibble(x=1:(n_var-1),y=(n_metric),
                        label=as.numeric(round(as_tibble(t(
                          colMeans(mean_imp[,4:ncol(mean_imp)])))[c(3,5,2,4,1)],2))),
            aes(x=x,y=y,label=paste0(label)),size=3.5,color=c("white",rep("black",4)))+
...
p_tot=ggarrange(p,p_tot_1,ncol=2,widths = c(.5,1),labels=letters[1:2])

ggsave("../Figures/Final_figs/LME_stability_spatial_struct_and_importance.pdf",p_tot,width=14,height = 4)
...
p1=ggplot(melting_d%>%
  ...
  geom_pointrange(aes(Size_tipping50,ymin=relativ_dis25,ymax=relativ_dis75,y=relativ_dis50,
                      fill=as.character(ID),color=as.character(ID)),
                  shape=21)+
  geom_pointrange(aes(Size_tipping50,xmin=Size_tipping25,xmax=Size_tipping75,y=relativ_dis50,
                      fill=as.character(ID),color=as.character(ID)),
                  shape=21)+
  the_theme+
  theme(legend.position = "none")+
  ...
  labs(y="Predicted distance to desert state",x="Predicted tipping point size")
...
p_tot=ggarrange(
  ggarrange(ggplot()+theme_void(),p1,ggplot()+theme_void(),ncol=3,
            labels=c("",letters[1],""),widths = c(.2,1.2,.2)),
  p_map,labels =c("",letters[2]) ,
...
## >> Fig. 5 Validation models ----

all_d_kefi=readRDS("../Data_new/Models_confirmation/Confirm_kefi/d_for_figure.rds")
...
for (k in 1:2){
  ...
  corr_sp=filter(d_spearman, Type_dist=="Rela",ID_sim==ifelse(k==1,1,3))
  
  d_fig=d_eby%>%add_column(., 
                           true_dist_abs=d_kefi$abs_dist,
                           true_dist_rela=d_kefi$relativ_dist)%>%
    filter(.,f ==unique(.$f)[c(1,3)][k])
  
  assign(paste0("p1_",k),
         ggplot(d_fig)+
           geom_pointrange(aes(x=true_dist_rela,y=relativ_dist,ymax=rela_q3,ymin=rela_q1),
                           color="black",fill="white",shape=23,lwd=1,size=1)+
           the_theme+
           theme(strip.text.x = element_blank())+
           labs(x="True distance",y="Inferred distance")+
...
p1=ggarrange(p1_1,p1_2,nrow=2)


all_d_guichard=readRDS("../Data_new/Models_confirmation/Confirm_Guichard/d_for_figure.rds")
...
for (a0_ in unique(d_eby$a0)){
  for (a2_ in unique(d_eby$a2)){
    
  corr_sp=filter(d_spearman, Type_dist=="Rela",ID_sim==id_plot)
  
  d_fig=d_eby%>%add_column(.,
                           true_dist_abs=d_guichard$abs_dist,
                           true_dist_rela=d_guichard$relativ_dist)%>%
    filter(.,a0 == a0_, a2 == a2_)
    
  assign(paste0("p2_",id_plot),
           ggplot(d_fig)+
           geom_pointrange(aes(x=true_dist_rela,y=relativ_dist,ymax=rela_q3,ymin=rela_q1),
                           color="black",fill="white",shape=23,lwd=1,size=1)+
           the_theme+
           ...
           ggtitle(paste0("r (Spearman) = ",round(median(corr_sp$Stat),2),
                          ...
                          ")"))+
           theme(title = element_text(size=10))+
           annotate("text",label=ifelse(id_plot%in%c(1,2),"Low disturbance",
                                        ifelse(id_plot%in%c(3,4),"Medium disturbance","High disturbance")),
                    x= min(d_fig$true_dist_rela) + pos_x * diff(range(d_fig$true_dist_rela)),
                    y=min(d_fig$relativ_dist) + pos_y1 * diff(range(d_fig$relativ_dist)))+
           annotate("text",label=ifelse(unique(d_fig$a2)==unique(d_eby$a2)[2],"High recovery","Low recovery"),
                    x= min(d_fig$true_dist_rela) + pos_x * diff(range(d_fig$true_dist_rela)),
                    y=min(d_fig$relativ_dist) + pos_y2 * diff(range(d_fig$relativ_dist)))
           )
...
p2=ggarrange(p2_1,p2_2,p2_3,
             p2_4,p2_5,p2_6,nrow=2,ncol=3)

ggsave("../Figures/Final_figs/Validation_using_simulations.pdf",
       ggarrange(p1,p2,widths = c(.25,.75),labels = letters[1:2],align = 'hv'),
       width = 17,height = 7)
...
ggsave("../Figures/Final_figs/SI/NMRSE_consistency_inference_param_scale.pdf",
...
for (k in 1:3){
  ...
  assign(paste0("p4_",k),
         ggplot(d_mod%>%melt(., measure.vars=c("AIC"))%>%filter(., Stability==unique(.$Stability)[k]))+
           geom_histogram(aes(x=value,fill=Predictor,group=Predictor))+
           geom_vline(data=d_mod%>%filter(., Stability==unique(.$Stability)[k])%>%
                        dplyr::group_by(., Predictor,Stability)%>%
                        dplyr::summarise(., .groups = "keep",median_AIC=median(AIC,na.rm = T)),
                      aes(xintercept=median_AIC),color="black")+
...
# >> 6) Posteriors: median and bimodality ----
...
ggsave("../Figures/Final_figs/SI/Posterior_median.pdf",p,width = 8,height = 4)
...
ggsave("../Figures/Final_figs/SI/Example_sites_distrib.pdf",
       ggarrange(ggarrange(
         ggarrange(labels = c("","A1",""),
                   ggplot()+theme_void(),
                   p_land1,ggplot()+theme_void(),ncol=3,widths =c(.3,1,.3)),
...
pdf("../Figures/Final_figs/SI/Kefi_C_dist_tipping.pdf",width = 8,height = 6)
{
  par(mfrow=c(2,3))
  id_plot=1
  ...
  for (f_ in unique(param_kefi$f)){
    ...
    corr_sp=filter(d_spearman, Type_dist=="Abs",ID_sim==id_plot)
    
    
    plot(d1$abs_dist,d2$abs_dist,col="gray",lwd=7,
         main="",xlab="",
         ylab="")
    errbar(d1$abs_dist,d2$abs_dist,d2$abs_q3,d2$abs_q1,
           col="#82A6E2",add=T)
    text(mean(d1$abs_dist)-diff(range(d1$abs_dist))/7,
         mean(d2$abs_dist)+diff(range(d2$abs_dist))/3,
         paste0("r = ",round(median(corr_sp$Stat),2),
                " (",round(quantile(corr_sp$Stat,.05),2),
                ", ",round(quantile(corr_sp$Stat,.95),2),
                ")"))
    text(mean(d1$abs_dist)+diff(range(d1$abs_dist))/7,
  ...
  mtext("Absolute distance", side = 3, line = -3, outer = TRUE)
  mtext("Relative distance", side = 3, line = -25, outer = TRUE)
  mtext("Mean dist from Eby model", side = 2, line = -1.5, outer = TRUE)
  mtext("Mean dist from Kefi model", side = 1, line = -2, outer = TRUE)
  ...
  for (f_ in unique(param_kefi$f)){
    ...
    plot(d1$relativ_dist,d2$relativ_dist,col="gray",lwd=7,pch=19,
         main="",
         xlab="",
         ylab="")
    errbar(d1$relativ_dist,d2$relativ_dist,
           d2$rela_q3,d2$rela_q1,
           col="#82A6E2",add=T)
    text(mean(d1$relativ_dist)-diff(range(d1$abs_dist))/7,
         mean(d2$relativ_dist)+diff(range(d2$relativ_dist))/3,
         paste0("r = ",round(median(corr_sp$Stat),2),
                " (",round(quantile(corr_sp$Stat,.05),2),
                ", ",round(quantile(corr_sp$Stat,.95),2),
                ")"))
    text(mean(d1$relativ_dist)+diff(range(d1$relativ_dist))/7,
         mean(d2$relativ_dist)-diff(range(d2$relativ_dist))/3,
         paste0("f = ",f_))
    id_plot=id_plot+1
...
x_y_stat=read.table("../Data_new/Models_confirmation/Confirm_kefi/x_y_stat.csv",sep=";")
...
p=annotate_figure(ggarrange(plotlist=list_plots,ncol = 4,nrow = 3,common.legend = T,legend="bottom"),
                  ...
                  bottom = text_grob("Stats in Kefi model (virtual observation)",color="black",size=15,face="bold",vjust=-1,family = "NewCenturySchoolbook"))
...
pdf("../Figures/Final_figs/SI/Guichard_C_dist_tipping.pdf",width = 8,height = 12)
{
  par(mfrow=c(4,3))
  id_plot=1
  for (a2_ in unique(param_guichard$a2)){
    ...
    mtext("Absolute distance", side = 3, line = -3, outer = TRUE)
    mtext("Relative distance", side = 3, line = -25, outer = TRUE)
    mtext("Absolute distance", side = 3, line = -47, outer = TRUE)
    mtext("Relative distance", side = 3, line = -70, outer = TRUE)
    mtext("Mean relat. dist from Eby model", side = 2, line = -1.5, outer = TRUE)
    mtext("Mean relat. dist from Guichard model", side = 1, line = -2, outer = TRUE)
    ...
    for (a0_ in unique(param_guichard$a0)){
      ...
      text(mean(d1$relativ_dist)-diff(range(d1$relativ_dist))/7,
           ...
           paste0("r = ",round(median(corr_sp$Stat),2),
                  ...
                  ")"))
      text(mean(d1$relativ_dist)+diff(range(d1$relativ_dist))/7,
           mean(d2$relativ_dist)-diff(range(d2$relativ_dist))/4,
           paste0("a0 = ",a0_,", a2 = ",a2_))
...
x_y_stat=read.table("../Data_new/Models_confirmation/Confirm_Guichard/x_y_stat.csv",sep=";")%>%
...
## Validating predictions using forest-gap model ----

#First the distance predicted by the forest-gap model


dist_kubo=tibble()
for (site in list.files("../Data_new/Models_confirmation/Confirm_kubo/Dist_kubo","Dist")){
  
  site_id=as.numeric(gsub(".csv","",strsplit(site,"_")[[1]][3]))
  
  pred=read.table(paste0("../Data_new/Models_confirmation/Confirm_kubo/Dist_kubo/",site),sep=",")%>%
    filter(., V1>0)
  colnames(pred)=c("d","delta","a","cover")
  pred$cover[pred$cover<.01 | pred$cover>.99]=0
  
  #to control for the size of the tipping point
  if (any(which(abs(diff(pred$cover))>.5))){
    pred=pred[-which(abs(diff(pred$cover))>.5),]
  }
  
  if (max(pred$cover)>.05){
    dist_kubo=rbind(dist_kubo,tibble(Site=site_id,
...
d_kubo=dist_kubo%>%
  add_column(., abs_q1=0,abs_q3=0,rela_q1=0,rela_q3=0,Model="Kubo")%>%
  arrange(., Site)%>%
  filter(., Cover<.9)


dist_eby=tibble();step_size=0.005
for (site in list.files("../Data_new/Models_confirmation/Confirm_kubo/Dist_Eby","Dist")){
  site_id=as.numeric(gsub("Eby","",gsub(".csv","",strsplit(site,"_")[[1]][3])))
  
  pred=read.table(paste0("../Data_new/Models_confirmation/Confirm_kubo/Dist_Eby/",site),sep=",")%>%
    filter(., V2>0)
  colnames(pred)=c("p","q","cover")
...
dist_eby=dist_eby%>%dplyr::arrange(., Site)%>%filter(., Cover !=0)%>%
  filter(., Cover <.9)%>%
  filter(., Site %in% unique(d_kubo$Site))
...
param_kubo=read.table("../Data_new/Models_confirmation/Confirm_kubo/Parameters_kubo.csv",sep=";")
colnames(param_kubo)=c("d","delta","a")
d_kubo=cbind(d_kubo,param_kubo[d_kubo$Site,])%>%
  filter(., Site %in% unique(d_eby$Site))
d_eby=cbind(d_eby,param_kubo[d_kubo$Site,])

d_spearman=tibble()
id=1;  n=200
for (delta_ in unique(param_kubo$delta)){
  for (a_ in unique(param_kubo$a)){
    
    
    for (x in 1:n){
      
      d1=dplyr::filter(d_kubo,delta==delta_,a==a_)
      d2=dplyr::filter(d_eby,delta==delta_,a==a_)
...
pdf("../Figures/Final_figs/SI/Kubo_C_dist_tipping.pdf",width = 6,height = 12)
{
  par(mfrow=c(4,2))
  id_plot=1
  for (delta_ in unique(param_kubo$delta)){
    ...
    mtext("Absolute distance", side = 3, line = -47, outer = TRUE)
    mtext("Relative distance", side = 3, line = -70, outer = TRUE)
    mtext("Mean relat. dist from Guichard model", side = 2, line = -1.5, outer = TRUE)
    mtext("Mean relat. dist from Eby model", side = 1, line = -2, outer = TRUE)
...
# comparizon on how much we fitted the statistics from the kubo forest-gap model

x_y_stat=read.table("../Data_new/Models_confirmation/Confirm_kubo/x_y_stat.csv",sep=";")%>%
  filter(., Site_ID %in% d_kubo$Site) #to avoid full covered landscapes
...
p=annotate_figure(ggarrange(plotlist=list_plots,ncol = 4,nrow = 3,common.legend = T,legend="bottom"),
                  left=text_grob("Stats in Contact process (closest selected simulations)",rot=90,color="black",size=15,face ="bold",vjust=1,family = "NewCenturySchoolbook"),
                  bottom = text_grob("Stats in Kubo model (virtual observation)",color="black",size=15,face="bold",vjust=-1,family = "NewCenturySchoolbook"))

ggsave("../Figures/Final_figs/SI/Kubo_B_xystats.pdf",p,width = 10,height = 8)
...
x_y_stat=read.table("../Data_new/Models_confirmation/Confirm_null/x_y_stat.csv",sep=";")
...
ggsave("../Figures/Final_figs/SI/Null_xystats.pdf",p,width = 10,height = 8)
...
# >> 10) Predictors of stability metrics ----
...
p_tot=ggarrange(p1_1,p1_2,p2_1,p2_2,nrow=2,ncol=2)
ggsave("../Figures/Final_figs/SI/Predictors_stability.pdf",p_tot,width = 8,height = 6)
